...
class Bolinha_pequena(ItaloSena):        
    def __init__(self, x, y, bolinhas_grandes, mapa, posicao_inicial):
        super().__init__(posicao_inicial)  # Inicializa ItaloSena
        self.posicoes = []
        self.mapa = mapa
    ...
    def verificar_colisao(self):
        to_remove = []
        for pos in self.posicoes:
            distancia = math.sqrt((pos[0] - self.posicao.x) ** 2 + (pos[1] - self.posicao.y) ** 2)
